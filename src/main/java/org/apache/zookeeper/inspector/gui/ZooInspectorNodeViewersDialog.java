 ...
 */
class ZooInspectorNodeViewersDialog extends JDialog implements
	 ...
	 * @param currentViewers - the {@link ZooInspectorNodeViewer}s to show
	 * @param listeners      - the {@link NodeViewersChangeListener}s which need to be
	 *                       notified of changes to the node viewers configuration
	 * @param manager        - the {@link ZooInspectorManager} for the application
	 */
	public ZooInspectorNodeViewersDialog(Frame frame,
		...
		super(frame);
		final List<ZooInspectorNodeViewer> newViewers = new ArrayList<>(
				currentViewers);
		...
		viewersList.setTransferHandler(new TransferHandler() {
			...
			protected Transferable createTransferable(JComponent c) {
				JList list = (JList) c;
				return (ZooInspectorNodeViewer) list
						.getSelectedValue();
			}
		});
		...
		removeButton = new JButton(ZooInspectorIconResources
				.getDeleteNodeIcon());
		JButton addButton = new JButton(ZooInspectorIconResources.getAddNodeIcon());
		...
		JButton setDefaultsButton = new JButton("Set As Defaults");
		saveFileButton
		...
		saveFileButton.addActionListener(e -> {
			...
			if (result == JFileChooser.APPROVE_OPTION) {
				...
				if (answer == JOptionPane.YES_OPTION) {
					DefaultListModel listModel = (DefaultListModel) viewersList
							.getModel();
					List<String> nodeViewersClassNames = new ArrayList<>();
				...
				try {
					List<String> nodeViewersClassNames = manager
							.loadNodeViewersFile(fileChooser
									.getSelectedFile());
					List<ZooInspectorNodeViewer> nodeViewers = new ArrayList<>();
					...
					for (ZooInspectorNodeViewer viewer : nodeViewers) {
						model1.addElement(viewer);
					}
					viewersList.setModel(model1);
					panel.revalidate();
					panel.repaint();
				} catch (Exception ex) {
					LoggerFactory
		...
		cancelButton.addActionListener(e -> ZooInspectorNodeViewersDialog.this.dispose());
