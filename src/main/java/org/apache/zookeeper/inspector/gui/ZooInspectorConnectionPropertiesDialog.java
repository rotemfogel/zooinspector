 ...
 */
class ZooInspectorConnectionPropertiesDialog extends JDialog {
	...
	public ZooInspectorConnectionPropertiesDialog(
		...
		for (Entry<String, List<String>> entry : connectionPropertiesTemplate
			...
			if (entry.getValue().size() == 0) {
				...
				JComboBox combo = new JComboBox(list.toArray(new String[0]));
		...
		loadPropsFileButton.addActionListener(e -> {
			...
			if (result == JFileChooser.APPROVE_OPTION) {
				...
				try {
					try (FileReader reader = new FileReader(propsFilePath)) {
						props.load(reader);
