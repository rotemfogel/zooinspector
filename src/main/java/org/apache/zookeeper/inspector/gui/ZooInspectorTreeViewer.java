...
//        System.out.println("selectedNodes: " + selectedPaths);
	...
	public void refreshViewAfterDelete(List<String> deletedNodes) {
		...
		Set<String> expandedNodes = new HashSet<>(getExpandedNodes());
		...
		for (String path : deletedNodes) {
			...
			try {
				zooInspectorManager.getCache().refresh(Collections.singletonList(parent), 0);
			} catch (Exception e) {
				zooInspectorPanel.checkZookeeperStates(e.getMessage());
	...
	public void refreshViewAfterAdd(String parent, String addNodeName) {
		...
		try {
			zooInspectorManager.getCache().refresh(Collections.singletonList(parent.isEmpty() ? "/" : parent), 0);
			zooInspectorManager.getCache().refresh(Collections.singletonList(parent + "/" + addNodeName), 0);
		} catch (Exception e) {
			zooInspectorPanel.checkZookeeperStates(e.getMessage());
...
//          System.out.println("TreeRender() called");
	...
	private class ZooInspectorTreeNode implements TreeNode {
		...
		ZooInspectorTreeNode(String nodePath, ZooInspectorTreeNode parent) {
			this.parent = parent;
