...
public class ZooInspectorManagerImpl implements ZooInspectorManager {
	 ...
	 */
	private static final String CONNECT_STRING = "hosts";
	/**
	 ...
	 */
	private static final String DATA_ENCRYPTION_MANAGER = "encryptionManager";
...
//      new File("./config/defaultConnectionSettings.cfg");
	...
	private ZooInspectorManagerCache cache;
...
//        System.out.println("[START] connected took: " + (System.currentTimeMillis() - start));
		...
		if (connected) {
			...
			try {
				cache.refresh(Collections.singletonList("/"), 1);
			} catch (Exception e) {
	...
	// let's try it
	private final ExecutorService service = Executors.newFixedThreadPool(40);
	
	Map<String, Item> getChildren(List<String> paths) {
		int n = paths.size();
		if (n > 0) {
			// final List<Item> ret = new ArrayList<Item>();
			final Map<String, Item> ret = new ConcurrentHashMap<>();
	...
	public String getData(String nodePath) {
		if (connected) {
			try {
				...
				if (s != null) {
					String data = this.encryptionManager.decryptData(zooKeeper.getData(nodePath, false, s));
					if (data != null) {
	...
	public List<Map<String, String>> getACLs(String nodePath) {
		List<Map<String, String>> returnACLs = new ArrayList<>();
		if (connected) {
			try {
				if (nodePath.length() == 0) {
					nodePath = "/";
	...
	public Map<String, String> getNodeMeta(String nodePath) {
		Map<String, String> nodeMeta = new LinkedHashMap<>();
		if (connected) {
			try {
				if (nodePath.length() == 0) {
					nodePath = "/";
	 ...
	 * (non-Javadoc)
	 *
	 * @see org.apache.zookeeper.inspector.manager.ZooInspectorReadOnlyManager#getSessionMeta()
	 */
	@Override
	public Map<String, String> getSessionMeta() {
		Map<String, String> sessionMeta = new LinkedHashMap<>();
		try {
			if (zooKeeper != null) {
	...
	public Pair<Map<String, List<String>>, Map<String, String>> getConnectionPropertiesTemplate() {
		...
		template.put(DATA_ENCRYPTION_MANAGER,
				Arrays.asList(defaultEncryptionManager));
	...
	class NodeWatcher implements Watcher {
		 ...
		 */
		NodeWatcher(String nodePath, NodeListener nodeListener, ZooKeeper zookeeper)
	...
	private void loadDefaultConnectionFile() throws IOException {
		...
		defaultHostsList = new ArrayList<>(Arrays.asList(defaultHosts.trim().split("\\s+")));
...
//    System.out.println("end");
	...
	private void initConfDir() throws IOException {
